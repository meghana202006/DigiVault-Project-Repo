...
function AudioSection() {
  ...
  const fetchFolders = useCallback(async () => {
    ...
    try {
      const response = await getFolders('audio')
      if (response && response.folders) setFolders(response.folders)
      else setFolders([])
    } catch (error) {
      console.error('Error fetching audio folders:', error)
      setFolders([])
    } finally {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const response = await getFiles('audio')
      if (response && response.files) setFiles(response.files)
      else setFiles([])
    } catch (error) {
      console.error('Error fetching audio files:', error)
      if (error.code !== 'ERR_NETWORK' && error.code !== 'ECONNABORTED') setFiles([])
    } finally {
  ...
  const handleFileDelete = useCallback((file) => {
    setDeleteToast({ fileName: file.name || file.originalName || 'File', fileId: file._id })
  ...
  return (
    <div ref={sectionRef} className="relative w-full">
      ...
      <div className="flex flex-col">
        ...
        <h2 className="text-white text-[28px] font-medium mt-10">Audio files</h2>
        {isLoadingFiles ? (
          <FolderListSkeleton count={3} />
