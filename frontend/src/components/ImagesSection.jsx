...
import { Image, X, CheckCircle2 } from 'lucide-react'
...
function ImagesSection() {
  ...
  const fetchFolders = useCallback(async () => {
    ...
    try {
      const response = await getFolders('image')
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const response = await getFiles('image')
  ...
  return (
    <div ref={sectionRef} className="relative w-full">
      ...
      <div className="flex flex-col">
        ...
        <h2 className="text-white text-[28px] font-medium mt-10">Images</h2>
        {isLoadingFiles ? (
          <FolderListSkeleton count={3} />
